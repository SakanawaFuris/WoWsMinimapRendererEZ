...
class BattleController(IBattleController):
    def __init__(self):
        ...
        Entity.subscribe_method_call(
            "Avatar", "receivePlayerData", self.receivePlayerData
        )
        Entity.subscribe_method_call(
        ...
        self._dict_score: dict[int, Score] = {0: Score(0, 0), 1: Score(1, 0)}
        ...
        Entity.subscribe_property_change(
            "Vehicle", "buoyancyCurrentState", self._set_buoyancy_state
        )
    ...
    def _update_caps(self, entity: Entity, cp_l):
        ...
        if cp["teamId"] == self._owner.get("teamId", -1) and cp["teamId"] != -1:
            relation = 0
        elif cp["teamId"] != self._owner.get("teamId", -1) and cp["teamId"] != -1:
            relation = 1
        else:
            relation = -1
    ...
    def _is_suppressed(self, entity: Entity, val):
        if entity.id in self._dict_building:
    ...
    def _set_regenerated_health(self, entity, health):
        if entity.id not in self._dict_vehicle:
            return
        self._dict_vehicle[entity.id] = self._dict_vehicle[entity.id]._replace(
    ...
    def _set_score(self, entity, score):
        if score["teamId"] == self._owner.get("teamId", -1):
            relation = 0
        else:
            relation = 1
    ...
    def _set_state(self, entity, state):
        ...
        for team_score in state["missions"]["teamsScore"]:
            if team_score["teamId"] == self._owner.get("teamId", -1):
                relation = 0
            else:
                relation = 1
    ...
    def _remove_ward(self, entity, plane_id):
        self._dict_ward.pop(plane_id, None)
    ...
    def _add_plane(
        ...
        try:
            ...
            if self._owner.get("teamId", -1) == team_id and team_id != -1:
                relation = 0
            elif self._owner.get("teamId", -1) != team_id and team_id != -1:
                relation = 1
            else:
                relation = -1
    ...
    def _remove_plane(self, entity: Entity, plane_id: int):
        self._dict_plane.pop(plane_id, None)

    def _on_consumable_used(self, entity: Entity, consumableUsageParams, workTimeLeft):
        consumable_type = consumableUsageParams[1] if len(consumableUsageParams) >= 2 else 0
        consumables = self._acc_consumables.setdefault(entity.id, [])
        consumables.append(
            Consumable(ship_id=entity.id, consumable_id=consumable_type, duration=workTimeLeft)
        )
    ...
    def _crew_skills(self, entity: Entity, params):
        try:
            pid = self._vehicle_to_id[entity.id]
            self._dict_info[pid] = self._dict_info[pid]._replace(
                skills=Skills(*params["learnedSkills"])
            )
        except KeyError:
            pass
    ...
    def _modernization(self, entity: Entity, config: bytes):
        try:
            ...
            logging.warning("_modernization parse failed for entity %s: %s", entity.id, e)
    ...
    def _create_player_vehicle_data(self):
        ...
        # Fallback: avatarId一致なしの場合、最初の人間プレイヤーをownerとする
        if not self._owner:
            for player in self._players.get_info().values():
                if player.get("playerType") == 1:
                    self._owner = player
                    break
        ...
        for player in self._players.get_info().values():
            ...
            is_ally = self._owner.get("teamId") == player.get("teamId")

            try:
                is_owner = self._owner["avatarId"] == player["avatarId"]
            except KeyError:
                is_owner = self._owner.get("avatarId") == player.get("id")
    ...
    def onSetConsumable(self, vehicle, blob):
        try:
            ...
            if vehicle.id in self._dict_vehicle:
                self._dict_vehicle[vehicle.id] = self._dict_vehicle[
                    vehicle.id
                ]._replace(consumables_state=consumables)
        except Exception as e:
            logging.warning("onSetConsumable failed for vehicle %s: %s", vehicle.id, e)
    ...
    def _ensure_vehicle(self, entity: Entity):
        """Create a placeholder Vehicle entry if it doesn't exist yet."""
        if entity.id not in self._dict_vehicle:
            self._dict_vehicle[entity.id] = Vehicle(
                player_id=entity.id,
                vehicle_id=entity.id,
                health=1,
                is_alive=True,
                x=-2500, y=-2500, yaw=-180,
                relation=1,
                is_visible=False,
                not_in_range=False,
                visibility_flag=0,
            ...
            self._vehicle_to_id.setdefault(entity.id, entity.id)

    def create_entity(self, entity: Entity):
        self._entities[entity.id] = entity

        if entity.get_name() == "Vehicle":
            # Always ensure a Vehicle entry exists so position updates can be tracked
            self._ensure_vehicle(entity)
            # Update relation based on entity properties
            try:
                owner_avatar_id = entity.properties["client"].get("owner", 0)
                team_id = entity.properties["client"].get("teamId", -1)
                if owner_avatar_id == self._player_id:
                    relation = -1
                    self._owner_team_id = team_id
                elif self._owner_team_id >= 0 and team_id == self._owner_team_id:
                    relation = 0
                elif self._owner_team_id >= 0 and team_id != self._owner_team_id and team_id >= 0:
                    relation = 1
                else:
                    relation = self._dict_vehicle[entity.id].relation
                self._dict_vehicle[entity.id] = self._dict_vehicle[entity.id]._replace(
                    relation=relation
                )
            except Exception:
        ...
        elif entity.get_name() == "SmokeScreen":
    ...
    def get_info(self):
        ...
        battle_time = (self._durations[-1] if self._durations else 0) - self._time_left + 1
        evt = Events(
            time_left=self._time_left,
            evt_vehicle=copy.copy(self._dict_vehicle),
            evt_building=copy.copy(self._dict_building),
        ...
        # Fallback: if binary parsing failed to populate _dict_info, build from _dict_vehicle
        if not self._dict_info and self._dict_vehicle:
            logging.warning(
                "_dict_info is empty - building PlayerInfo from _dict_vehicle "
                "(%d vehicles)", len(self._dict_vehicle)
            )
            owner_team = self._owner_team_id if self._owner_team_id >= 0 else 0
            for vehicle_id, vehicle in self._dict_vehicle.items():
                team_id = owner_team if vehicle.relation in (-1, 0) else (1 - owner_team)
                pi = PlayerInfo(
                    id=vehicle_id,
                    account_db_id=0,
                    clan_color=0, clan_id=0, clan_tag='',
                    max_health=50000,
                    name=f"Player_{vehicle_id}",
                    realm='',
                    ship_id=vehicle_id,   # BigWorld entity ID — matches evt_vehicle keys
                    team_id=team_id,
                    is_bot=False,
                    ship_params_id=0,
                    relation=vehicle.relation,
                    hull=None, abilities=(), modernization=(), ship_components={},
                )
                self._dict_info[vehicle_id] = pi
            if not self._owner:
                owner_vehicle = next(
                    (v for v in self._dict_vehicle.values() if v.relation == -1),
                    next(iter(self._dict_vehicle.values()), None)
                )
                if owner_vehicle is not None:
                    ov_id = owner_vehicle.vehicle_id
                    self._owner = {
                        "id": ov_id,
                        "avatarId": self._player_id or ov_id,
                        "shipId": ov_id,
                        "teamId": owner_team,
                    }
        ...
        rd = ReplayData(
            ...
            owner_avatar_id=self._owner.get("avatarId", self._player_id or 0),
            owner_vehicle_id=self._owner.get("shipId", 0),
            owner_id=self._owner.get("id") or next(iter(self._dict_info), self._player_id or 0),
    ...
    def onArenaStateReceived(
        ...
        try:
            players_list = restricted_loads(playersStates, encoding="latin1")
        except Exception as e:
            logging.warning("playersStates decode failed: %s", e)
            players_list = []
        self._players.create_or_update_players(players_list, PlayerType.PLAYER)
        try:
            self._players.create_or_update_players(
                restricted_loads(botsStates, encoding="latin1"), PlayerType.BOT
            )
        except Exception as e:
            logging.warning("botsStates decode failed: %s", e)
        try:
            self._players.create_or_update_players(
            ...
            logging.warning("buildingsInfo decode failed: %s", e)
        try:
            self._players.create_or_update_players(
            ...
            logging.warning("observersState decode failed: %s", e)

        self._create_player_vehicle_data()

    def receivePlayerData(self, avatar, blob, is_update):
        pass
    ...
    def receiveDamageStat(self, avatar, blob):
        try:
            ...
            logging.warning("receiveDamageStat failed: %s", e)
