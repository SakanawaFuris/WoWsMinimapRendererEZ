...
class LayerFragBase(LayerBase):
    ...
    def draw(self, game_time: int, image: Image.Image):
        ...
        for frag in reversed(self._frags[-5:]):
            ...
            f_ship = self._ships.get(fragger_info.ship_params_id)
            if f_ship:
                f_name = f_ship["name"]
                ...
                f_name, f_species, f_level = "???", "Cruiser", 5
                f_unknown = True

            k_ship = self._ships.get(killed_info.ship_params_id)
            if k_ship:
                k_name = k_ship["name"]
                ...
                k_name, k_species, k_level = "???", "Cruiser", 5
                k_unknown = True
