...
class LayerShotBase(LayerBase):
    ...
    def draw(self, game_time: int, image: Image.Image):
        ...
        projectiles.sort(
            key=lambda o: self._projectiles_data.get(o[1], "AP"), reverse=True
        )
        ...
        for projectile in projectiles:
            try:
                ...
                if self._renderer.team_tracers:
                    ...
                    if params_id in self._projectiles_data:
                        shell_type = self._projectiles_data[params_id]
                        color = SHELL_COLORS.get(shell_type, SHELL_COLORS["AP"])
                    else:
                        color = SHELL_COLORS["UNKNOWN"]
