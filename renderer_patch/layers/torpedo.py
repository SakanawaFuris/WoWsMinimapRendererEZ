...
TRAIL_MAXLEN = 300  # maximum number of trail points per torpedo (~5 min at 1fps)
...
class LayerTorpedoBase(LayerBase):
    ...
    def __init__(
        ...
        self._torpedo_trails: dict[int, deque] = {}
    ...
    def draw(self, game_time: int, draw: ImageDraw.ImageDraw):
        ...
        for hit in self._hits.copy():
            try:
                ...
                self._torpedo_trails.pop(hit, None)
            except KeyError:
                pass
            else:
                pass
        ...
        for sid, torpedo in events.evt_torpedo.items():
            ...
            if sid not in self._torpedo_trails:
                self._torpedo_trails[sid] = deque([(x, y)], maxlen=TRAIL_MAXLEN)
        ...
        for sid, active_torpedo in self._active_torpedoes.items():
            ...
            trail = self._torpedo_trails.get(sid)
            if trail and len(trail) >= 2:
                draw.line(list(trail), fill=color, width=1)
