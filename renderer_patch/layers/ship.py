    ...
    def draw(self, game_time: int, image: Image.Image):
        ...
        for vehicle in sorted(
            ...
            try:
                ...
                if vehicle.player_id not in self._holders or vehicle.player_id not in self._replay_data.player_info:
                    continue

                holder = self._holders[vehicle.player_id]
                player = self._replay_data.player_info[vehicle.player_id]
                ship = self._ships.get(player.ship_params_id)
                if ship is None:
                    print(f"[ship skip] unknown ship_params_id: {player.ship_params_id} player={player.name}")
                    ship = {"species": "Cruiser", "level": 5}
                ...
                if vehicle.is_visible and vehicle != owner_vehicle:
                    ...
                    owner_ship = self._ships.get(self._owner.ship_params_id, {"level": 5})
                    owner_view_range *= 1.25
                    owner_view_range = max(
                        owner_view_range, MIN_VIEW_DISTANCES.get(owner_ship["level"], 35000)
                    )
                ...
                if vehicle.is_alive and not self._renderer.dual_mode:
                    if not vehicle.is_visible:
                        # Off-map (pos=-2500,-2500): completely invisible, skip drawing
                        continue
                    if relation == 1 and not vehicle.visibility_flag and is_in_view_range:
                    ...
                    if vehicle.is_visible:
                        ...
                        if is_in_view_range:
                            draw_health_bar(
                                ...
                                color=color,
                                hp_per=round(
                                    vehicle.health / max(player.max_health, 1), 2
                                ),
                            )
                            if ship.get("species") == "Submarine":
                                draw_depth_bar(
                                    holder,
                                    depth_state=vehicle.buoyancy_state,
                                )
