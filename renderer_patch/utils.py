...
def generate_holder(
    ...
    for player in player_info.values():
        ...
        info = ships.get(player.ship_params_id, {"name": f"Unknown({player.ship_params_id})", "species": "Cruiser", "level": 1, "nation": "unknown"})
...
def draw_depth_bar(
    image: Image.Image, width=50, height=5, y_pos=57, depth_state=0, max_state=3
):
    """Draws a depth indicator bar for submarines.

    Args:
        image (Image.Image): The holder image.
        width (int): Width of the bar. Defaults to 50.
        height (int): Height of the bar. Defaults to 5.
        y_pos (int): Y position. Defaults to 57 (above health bar at 65).
        depth_state (int): Current buoyancy state (0=surface, higher=deeper).
        max_state (int): Maximum buoyancy state value. Defaults to 3.
    """
    if depth_state <= 0:
        return
    draw = ImageDraw.Draw(image)
    x1 = image.width / 2 - width / 2
    y1 = y_pos
    fill_ratio = min(depth_state, max_state) / max_state
    x2 = ((image.width / 2 + width / 2) - x1) * fill_ratio + x1
    x_max = image.width / 2 + width / 2
    y2 = y_pos + height
    draw.rectangle((x1, y1, x2, y2), fill="#3399FF")
    draw.rectangle((x1, y1, x_max, y2), outline="#3399FF", width=1)
